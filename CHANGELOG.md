...
- YAML pattern catalog (13 seed AWS pattern families, 18 entries) embedded in the binary,
...
- `--version` reports the build version — injected via `-ldflags` for `make build`
  and release binaries, and read from Go's module build info for `go install`
  builds.
...
- Conservative-safety hardening: exact-precision
  ...
  statement (the two forms match different request sets — `{"AWS":"*"}` only
  matches a request signed by some AWS principal — so under Deny the collapse
  could stop a statement denying anonymous access, e.g. a `DenyInsecureTransport`);
  trailing-JSON rejection; whole-object boolean sensitivity
...
### Known limitations
- AWS provider only; seed catalog of 13 pattern families; top-level attribute
